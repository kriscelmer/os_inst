...
echo "---> Installing nova on compute3"
set -e
set -x
DEBIAN_FRONTEND=noninteractive apt-get install -y nova-compute qemu-kvm > /dev/null
crudini --set /etc/nova/nova.conf DEFAULT transport_url 'rabbit://openstack:openstack@controller:5672/'
crudini --set /etc/nova/nova.conf DEFAULT my_ip 10.0.0.33
...
set +x
echo "---> Nova installed on compute3"
