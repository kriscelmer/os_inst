...
echo "---> Installing Swift on storage1 node"
set -e
set -x
...
crudini --set /etc/swift/account-server.conf DEFAULT bind_ip 10.0.0.41
...
crudini --set /etc/swift/container-server.conf DEFAULT bind_ip 10.0.0.41
...
crudini --set /etc/swift/object-server.conf DEFAULT bind_ip 10.0.0.41
...
curl -o /etc/swift/internal-client.conf https://opendev.org/openstack/swift/raw/branch/stable/caracal/etc/internal-client.conf-sample
